...
export function CategoryPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold capitalize text-foreground">{displayName}</h1>
        {currentCategory?.description && (
          <p className="mt-2 text-muted-foreground max-w-2xl">{currentCategory.description}</p>
        )}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {products.map(product => (
            <Link key={product._id} to={`/products/${product.slug}`}>
              <Card className="overflow-hidden hover:shadow-lg transition-shadow h-full group border-border">
